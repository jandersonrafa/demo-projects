...
# Resource variables
variable "php_laravel_fpm_monolith_cpu" { type = number }
variable "php_laravel_fpm_monolith_mem" { type = number }
variable "php_laravel_fpm_gateway_cpu" { type = number }
variable "php_laravel_fpm_gateway_mem" { type = number }
...
job "php-laravel-fpm" {
  ...
  group "monolith" {
    count = var.php_laravel_fpm_monolith_count
    ...
    task "app" {
      ...
      config {
        ...
        memory_hard_limit = var.php_laravel_fpm_monolith_mem
        cpu_hard_limit    = true
      }
      env {
        DB_CONNECTION = "pgsql"
      ...
      resources {
        ...
        memory = var.php_laravel_fpm_monolith_mem
      }
    }
...
EOF
  ...
  group "gateway" {
    count = var.php_laravel_fpm_gateway_count
    ...
    task "app" {
      ...
      config {
        ...
        command = "sh"
        args    = ["-c", "echo 'listen = 0.0.0.0:${NOMAD_PORT_fpm}' >> /usr/local/etc/php-fpm.d/zz-nomad.conf && php-fpm"]
        memory_hard_limit = var.php_laravel_fpm_gateway_mem
        cpu_hard_limit    = true
      }
      env {
      ...
      resources {
        ...
        memory = var.php_laravel_fpm_gateway_mem
      }
    }
