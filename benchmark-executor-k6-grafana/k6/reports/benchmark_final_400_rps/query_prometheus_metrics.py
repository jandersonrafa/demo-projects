...
TEST_WINDOWS = {
    ...
    "php-fpm": {
        "start": "2026-01-21T15:49:45-03:00",
        "port": 3011,
        "stack": "php-fpm",
        "name": "PHP FPM"
    },
    "python": {
        "start": "2026-01-21T16:01:43-03:00",
        "port": 3008,
        "stack": "python",
        "name": "Python (FastAPI)"
    },
    "node": {
        "start": "2026-01-21T16:12:47-03:00",
        "port": 3005,
        "stack": "node",
        "name": "Node.js (NestJS)"
    },
    "java-mvc-vt": {
        "start": "2026-01-21T16:24:29-03:00",
        "port": 3007,
        "stack": "java-mvc-vt",
        "name": "Java MVC VT"
    },
    "java-webflux": {
        "start": "2026-01-21T16:35:55-03:00",
        "port": 3006,
        "stack": "java-webflux",
        "name": "Java WebFlux"
    }
...
def get_container_metrics(stack_key, test_info):
    ...
    rps_query = f'sum(rate(http_requests_total{{job=~".*{stack_label}.*gateway.*", handler!~"/actuator.*|/metrics", path!~"/actuator.*|/metrics"}}[1m]))'
    print(f"\nRPS Query (http_requests_total): {rps_query}")
    rps_data = query_prometheus(rps_query, start_ts, end_ts)
    
    rps_values = []
    if rps_data and rps_data.get("status") == "success" and rps_data["data"]["result"]:
        for result in rps_data["data"]["result"]:
            for value in result["values"]:
                rps_values.append(float(value[1]))
    
    # If no data, try http_server_requests_seconds_count (Java Spring Boot alternative)
    if not rps_values:
        rps_query_alt = f'sum(rate(http_server_requests_seconds_count{{job=~".*{stack_label}.*gateway.*", uri!~"/actuator.*|/metrics"}}[1m]))'
        print(f"RPS Query (http_server_requests_seconds_count): {rps_query_alt}")
        rps_data_alt = query_prometheus(rps_query_alt, start_ts, end_ts)
        
        if rps_data_alt and rps_data_alt.get("status") == "success" and rps_data_alt["data"]["result"]:
            for result in rps_data_alt["data"]["result"]:
                for value in result["values"]:
                    rps_values.append(float(value[1]))
